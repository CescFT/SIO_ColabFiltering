    ...
    for count in range(10):
        ...
        for i in range(100):
            ...
            for num_user in range(user_group_id.size):
                ...
                rating_user = pivot_ratings.loc[[user_group_id['userID'].values[num_user]] , : ].stack(dropna=False).to_numpy()
                ...
                if pred_user['rating'].isna().sum() != 0:
                    ...
                    for x in np.append(np.arange(0, num_user), np.arange(num_user+1, user_group_id.size)):
                        rating_user = pivot_ratings.loc[[user_group_id['userID'].values[x]] , : ].stack(dropna=False).to_numpy()
