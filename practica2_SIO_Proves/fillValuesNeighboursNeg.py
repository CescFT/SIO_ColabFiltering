...
try:
    ...
    for i in range(user_neg['userID'].size):
        print(str(i) + ':' + str(user_neg['userID'].size))
        user_id = int(user_neg.loc[i]['userID'])
        ...
        if i % 10 == 0 and i != 0 and i + 10 < user_neg['userID'].size - 1:
