...
try:
    ...
    user_neg_aux = user_pos.tail(4)
    user_neg_aux = user_neg_aux.reset_index(drop=True)
    ...
    for i in range(user_neg_aux['userID'].size):
        print(str(i) + ':' + str(user_neg_aux['userID'].size))
        user_id = int(user_neg_aux.loc[i]['userID'])
        ...
        # if i % 10 == 0 and i != 0 and i + 10 < user_neg_aux.size - 1:

        if i+1 == user_neg_aux['userID'].size:
            ...
            if True:
                ...
                template_df_nan.to_csv('C:\\Users\\Sancho\\Desktop\\predicted_negative.csv', mode='a', index=False,
                                       header=False)
                template_df_nan = []
                template_df_concat_dict = []
        '''
        elif i == user_neg_aux.size - 1:
            template_df_concat = pd.DataFrame.from_dict(template_df_concat_dict)
            template_df_concat['isnan'] = template_df_concat['rating'].apply(lambda x: np.isnan(x))
            template_df_concat.reset_index(drop=True, inplace=True)
            lon = i - x
    ...
    template_df_nan['restaurantID'] = 'Restaurant' + template_df_nan['restaurantID'].astype(str)
    template_df_nan.drop('isnan', axis=1, inplace=True)
    template_df_nan.to_csv('C:\\Users\\Sancho\\Desktop\\predicted_negative.csv', mode='a', index=False, header=False)
    '''
